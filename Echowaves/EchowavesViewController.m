        ...
        [library enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            [group enumerateAssetsAtIndexes:[NSIndexSet indexSetWithIndex:([group numberOfAssets]-1)] //change this for latest photo
                                 ...
                                 usingBlock:^(ALAsset *alAsset, NSUInteger index, BOOL *innerStop) {
                                     ...
                                     if (alAsset)
                                     {
                                         NSDate *lastAssetDateTime = [alAsset valueForProperty:ALAssetPropertyDate];
                                         NSDate *currentDateTime = [NSDate date];
                                         NSTimeInterval timeSinceLastPost = [currentDateTime timeIntervalSinceDate:lastAssetDateTime]; // diff
                                         NSLog(@"interval since last post: %f", timeSinceLastPost);
                                         if(timeSinceLastPost <= 2.0) {//this means a new image appeared since last cycle -- time to post
                                             ...
                                             NSString *dateString = [formatter stringFromDate:currentDateTime];
